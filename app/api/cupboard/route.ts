...
export async function GET(request: Request) {
  //function to get all needs for a specific admin
  try {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Need created successfully" }),
      {
        status: 201,
        headers: {
          "Content-Type": "application/json",
...
export async function PUT(request: Request) {
  //function to change the data about existing needs
  try {
    const { need }: { need: Need } = await request.json();
    if (!need || !need.id) {
      return new Response(
        JSON.stringify({ message: "Missing need or need ID" }),
        {
          status: 400,
          headers: {
            "Content-Type": "application/json",
    ...
    return new Response(
      JSON.stringify({ message: "Need updated successfully" }),
      {
        status: 200,
        headers: {
          "Content-Type": "application/json",
...
export async function DELETE(req: Request) {
  //deletes specific need from the cupboard
  try {
