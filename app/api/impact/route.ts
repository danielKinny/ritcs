...
export async function GET() {
  ...
  const [rows] = await db.execute("SELECT * FROM needs"); //return all needs

  const needs = Array.from(rows as Array<Need>);
  //calculate stats after typecasting
  const totalNeeded = needs.reduce(
    (acc, need) => acc + Number(need.amountNeeded),
    0
  );
  const totalDonated = needs.reduce(
    (acc, need) => acc + Number(need.amountDonated),
    0
  );
