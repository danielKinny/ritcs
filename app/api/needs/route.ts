...
export async function PUT(request: Request) {
  //used to close a need, only can be done by admin
  try {
    const needID: number = Number(
      new URL(request.url).searchParams.get("needID")
    );
    const db = await databaseConnection();

    await db.execute("UPDATE needs SET closed = true WHERE id = ?", [needID]);
...
export async function GET(request: Request) {
  //fetch all needs, except those in basket
  try {
    ...
    if (basketID) {
      ...
      basketNeedIDs = Array.isArray(needqres)
        ? needqres.map((row) => (row as { needID: number }).needID)
        : [];
    }

    const [rows] = await db.execute("SELECT * FROM needs");
    const needs: Need[] = (Array.isArray(rows) ? (rows as Need[]) : []).filter(
      (need) => Number(need.amountNeeded) > Number(need.amountDonated)
    );
