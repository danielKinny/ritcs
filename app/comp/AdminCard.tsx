  ...
  const handleDelete = async () => {
    try {
      ...
      if (res.ok) {
        ...
        toast.error(
          `Error deleting need: ${errorData.error || "Unknown error"}`
        );
      }
    } catch (error) {
  ...
  const handleClose = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json();
        toast.error(
          `Error closing need: ${errorData.error || "Unknown error"}`
        );
      }
    } catch (error) {
    ...
    >
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="flex gap-2">
          ...
          <button
            className="px-3 py-1 text-sm border rounded border-gray-200 hover:bg-gray-50"
            onClick={() => onEdit?.(need)}
          >
            Edit
          </button>
