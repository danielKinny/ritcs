...
const CupboardPage = () => {
    ...
    const fetchNeeds = async (refresh = false) => {
        ...
        try {
            ...
            setNeedsCreated((Array.isArray(data) ? data : []).filter((need) => need.closed === null || need.closed === 0));
        } catch (e: any) {
    ...
    const stats = useMemo(() => {
        ...
        const totalNeeded = Number(needsCreated.reduce(
            (sum, n) => sum + (Number(n.amountNeeded) || 0),
            0
        ));
        const totalDonated = Number(needsCreated.reduce(
            ...
            <header className="max-w-7xl mx-auto mb-8">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            onClick={() => { setEditNeed(null); setShowModal(true); }}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
                    ...
                    <div className="rounded-lg border p-4 bg-white">
                        ...
                        <div className="text-xl font-semibold text-red-600">{stats.high}</div>
                    </div>
                    <div className="rounded-lg border p-4 bg-white">
                        <div className="text-xs text-gray-500">Total donations collected</div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {filtered.map((need) => (
                                    <AdminCard
                                        ...
                                        onEdit={(n) => { setEditNeed(n); setShowModal(true); }}
